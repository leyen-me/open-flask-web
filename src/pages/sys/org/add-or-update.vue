  ...
  <el-dialog v-model="visible" :title="!dataForm.id ? '新增' : '修改'" :close-on-click-modal="false" draggable>
    <el-form ref="dataFormRef" :model="dataForm" :rules="dataRules" label-width="120px" @keyup.enter="submitHandle()">
      ...
      <el-form-item prop="parent_name" label="上级机构" class="org-list">
        <el-popover ref="orgListPopover" placement="bottom-start" trigger="click" :width="400"
                    popper-class="popover-pop">
          <template #reference>
            <el-input v-model="dataForm.parent_name" :readonly="true" placeholder="上级机构">
              <template #suffix>
                <BaseSvgIcon v-if="dataForm.pid !== 0" icon="icon-close-circle"
                             @click.stop="treeSetDefaultHandle()"></BaseSvgIcon>
              </template>
            </el-input>
          </template>
